...
export class UserCommand extends Command {
    ...
    public async messageRun(message: Message, args: Args) {
        ...
        const index = await guild?.members.fetch()
            .then(e => e.sort((userA, userB) => userA.joinedTimestamp! - userB.joinedTimestamp!)
                .map(user => user.id)
        ...
        if (roles) {
            sortedRoles = roles.sort((roleA, roleB) => roleB.position - roleA.position)
                .map(e => `<@&${e.id}>`)
        ...
        var embed = new MessageEmbed()
            ...
            .setDescription(`Status: ${tempEmoji! ?? ''} ${status?.state ?? `${state?.toString().charAt(0).toUpperCase()}${state?.toString().slice(1)}`}
                \n${`Playing: ${playing}` ?? ''}`)
    ...
    public async chatInputRun(interaction: Command.ChatInputInteraction) {
        ...
        var embed = new MessageEmbed()
            .setTitle(`${guildUser?.nickname} (${user.tag})` ?? `${user.tag}`)
            .setDescription(`Status: ${tempEmoji! ?? ''} ${status?.state ?? `${state?.toString().charAt(0).toUpperCase()}${state?.toString().slice(1)}`}${isPlayingString}`)
            .setThumbnail(guildUser?.displayAvatarURL({ dynamic: true }) ?? user.displayAvatarURL({ dynamic: true }))
            .setColor(guildUser?.displayColor as ColorResolvable)
            .addFields(
                { name: `Joined Discord on`, value: user.createdAt.toLocaleString('ja-JP'), inline: true },
